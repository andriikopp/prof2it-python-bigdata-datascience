...
def get_people_and_characters_played(spark):
    ...
    return spark.sql("""
                     SELECT name_basics.primaryName, title_basics.primaryTitle, title_principals.characters 
                     FROM (title_basics INNER JOIN title_principals ON title_basics.tconst = title_principals.tconst) 
                        INNER JOIN name_basics ON name_basics.nconst = title_principals.nconst 
                     WHERE title_principals.category IN ('actor', 'actress')
                     """)
...
def get_top_100_adult_movies_per_region(spark):
    ...
    return spark.sql("""
                     SELECT title_akas.region, COUNT(title_basics.tconst) AS total 
                     FROM title_akas INNER JOIN title_basics ON title_basics.tconst = title_akas.titleId 
                     WHERE title_basics.isAdult = 1 
                     GROUP BY title_akas.region 
                     ORDER BY total DESC 
                     LIMIT 100
                     """)
...
def get_top_50_tvseries_by_episodes(spark):
    ...
    return spark.sql("""
                     SELECT title_basics.primaryTitle, COUNT(title_episode.tconst) as episodes
                     FROM title_basics INNER JOIN title_episode ON title_basics.tconst = title_episode.parentTconst
...
def get_10_most_popular_titles_by_each_decade(spark):
    ...
    return spark.sql("""
                     SELECT *
                     FROM (
                         SELECT CONCAT(FLOOR(startYear / 10) * 10, ' - ', FLOOR(startYear / 10) * 10 + 9) as decade, title_basics.primaryTitle, title_ratings.averageRating, ROW_NUMBER() OVER (PARTITION BY title_basics.primaryTitle ORDER BY title_ratings.averageRating DESC) AS num_titles
                         FROM title_basics INNER JOIN title_ratings ON title_basics.tconst = title_ratings.tconst
                     ) AS sub_query
                     WHERE num_titles == 10
                     ORDER BY decade DESC, averageRating DESC
                     """)
...
def get_10_most_popular_titles_by_each_genre(spark):
    ...
    return spark.sql("""
                     SELECT *
                     FROM (
                         SELECT SPLIT(title_basics.genres, ',') as genre, title_basics.primaryTitle, title_ratings.averageRating, ROW_NUMBER() OVER (PARTITION BY title_basics.primaryTitle ORDER BY title_ratings.averageRating DESC) AS num_titles
                         FROM title_basics INNER JOIN title_ratings ON title_basics.tconst = title_ratings.tconst
                     ) AS sub_query
                     WHERE num_titles == 10
                     ORDER BY genre DESC, averageRating DESC
                     """)
...
def main():
    ...
    # Extraction
    for name, file in data_sets.keys():
